class DataExportService {
  ...
  async exportToPDF(data, filename = null) {
    try {
      ...
      if (!filename) {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        filename = `data-backup-${timestamp}.pdf`;
      }
      ...
      doc.text(`Version: ${data.version || '1.0.0'}`, pageWidth / 2, yPosition + 5, { align: 'center' });
      yPosition += 15;
      ...
      const summaryData = [
        ...
        `Financial Transactions: ${data.transactions?.length || 0}`,
        `Budgets: ${data.budgets?.length || 0}`,
        `Financial Goals: ${data.financialGoals?.length || 0}`,
        `Bills: ${data.bills?.length || 0}`,
        `Loans: ${data.loans?.length || 0}`,
        `Loan Payments: ${data.loanPayments?.length || 0}`
      ];
      ...
      if (data.bookmarks && data.bookmarks.length > 0) {
        ...
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.text('Bookmarks', margin, yPosition);
        yPosition += 10;

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        data.bookmarks.slice(0, 10).forEach((bookmark, index) => {
          checkPageBreak(lineHeight * 3);
          doc.text(`${index + 1}. ${bookmark.title || 'No Title'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   URL: ${bookmark.url || 'No URL'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Category: ${bookmark.category || 'No Category'}`, margin + 5, yPosition);
          yPosition += lineHeight + 3;
        });

        if (data.bookmarks.length > 10) {
          checkPageBreak(lineHeight);
          doc.text(`... and ${data.bookmarks.length - 10} more bookmarks`, margin + 5, yPosition);
      ...
      if (data.todos && data.todos.length > 0) {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        data.todos.slice(0, 10).forEach((todo, index) => {
          checkPageBreak(lineHeight * 3);
        ...
        if (data.todos.length > 10) {
          checkPageBreak(lineHeight);
          doc.text(`... and ${data.todos.length - 10} more todos`, margin + 5, yPosition);
          yPosition += lineHeight + 5;
        } else {
          yPosition += 5;
        }
      ...
      if (data.books && data.books.length > 0) {
        ...
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.text('Books', margin, yPosition);
        yPosition += 10;

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        data.books.slice(0, 8).forEach((book, index) => {
          checkPageBreak(lineHeight * 4);
          doc.text(`${index + 1}. ${book.title || 'No Title'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Author: ${book.author || 'Unknown Author'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Status: ${book.status || 'reading'} | Progress: ${book.currentPage || 0}/${book.totalPages || 0} pages`, margin + 5, yPosition);
          yPosition += lineHeight;
          if (book.genre) {
            doc.text(`   Genre: ${book.genre}`, margin + 5, yPosition);
            yPosition += lineHeight;
          }
          yPosition += 2;
        });
        ...
        if (data.books.length > 8) {
          checkPageBreak(lineHeight);
          doc.text(`... and ${data.books.length - 8} more books`, margin + 5, yPosition);
          yPosition += lineHeight + 5;
        } else {
          yPosition += 5;
        }
      ...
      if (data.habits && data.habits.length > 0) {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        data.habits.slice(0, 8).forEach((habit, index) => {
          checkPageBreak(lineHeight * 3);
          doc.text(`${index + 1}. ${habit.name || 'No Name'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Category: ${habit.category || 'personal'} | Frequency: ${habit.frequency || 'daily'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Streak: ${habit.streak || 0} days | Target: ${habit.targetCount || 1} per day`, margin + 5, yPosition);
          yPosition += lineHeight + 3;
        });

        if (data.habits.length > 8) {
          checkPageBreak(lineHeight);
          doc.text(`... and ${data.habits.length - 8} more habits`, margin + 5, yPosition);
          yPosition += lineHeight + 5;
        } else {
          yPosition += 5;
        }
      ...
      if (data.transactions && data.transactions.length > 0) {
        ...
        doc.text(`Total Income: ${this.formatCurrency(totalIncome)}`, margin + 5, yPosition);
        yPosition += lineHeight;
        doc.text(`Total Expenses: ${this.formatCurrency(totalExpenses)}`, margin + 5, yPosition);
        yPosition += lineHeight;
        doc.text(`Net Income: ${this.formatCurrency(netIncome)}`, margin + 5, yPosition);
        yPosition += lineHeight + 5;

        // Recent transactions
        doc.text('Recent Transactions:', margin + 5, yPosition);
        yPosition += lineHeight;
        
        data.transactions.slice(0, 8).forEach((transaction, index) => {
          checkPageBreak(lineHeight * 2);
          const amount = transaction.amount || 0;
          const type = amount > 0 ? '+' : '-';
          doc.text(`   ${type} ${this.formatCurrency(Math.abs(amount))} - ${transaction.description || 'No description'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   ${transaction.date || 'No date'} | ${transaction.category || 'No category'}`, margin + 5, yPosition);
          yPosition += lineHeight + 2;
        });

        if (data.transactions.length > 8) {
          checkPageBreak(lineHeight);
          doc.text(`... and ${data.transactions.length - 8} more transactions`, margin + 5, yPosition);
          yPosition += lineHeight + 5;
        } else {
          yPosition += 5;
        }
      ...
      if (data.loans && data.loans.length > 0) {
        ...
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.text('Loans', margin, yPosition);
        yPosition += 10;

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        data.loans.slice(0, 6).forEach((loan, index) => {
          checkPageBreak(lineHeight * 4);
          const loanType = loan.type === 'given' ? 'Given to' : 'Received from';
          doc.text(`${index + 1}. ${loanType}: ${loan.personName || 'Unknown'}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Principal: ${this.formatCurrency(loan.principalAmount || 0)} | Balance: ${this.formatCurrency(loan.currentBalance || 0)}`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Status: ${loan.status || 'active'} | Interest Rate: ${loan.interestRate || 0}%`, margin + 5, yPosition);
          yPosition += lineHeight;
          doc.text(`   Start Date: ${loan.startDate || 'Unknown'} | Due Date: ${loan.dueDate || 'No due date'}`, margin + 5, yPosition);
          yPosition += lineHeight + 3;
        });

        if (data.loans.length > 6) {
          checkPageBreak(lineHeight);
          doc.text(`... and ${data.loans.length - 6} more loans`, margin + 5, yPosition);
          yPosition += lineHeight + 5;
        } else {
          yPosition += 5;
        }
      ...
      doc.text('This report was generated automatically from your personal data management application.', margin, pageHeight - 15);
      ...
      this.saveToExportHistory({
        filename,
        format: 'pdf',
        size: 0, // PDF size is calculated differently
        timestamp: new Date().toISOString(),
        dataTypes: this.getDataTypesIncluded(data)
      });
