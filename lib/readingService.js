class ReadingService {
  ...
  // Books operations
  getBooks() {
    console.log('📚 ReadingService: getBooks() called');
    try {
      const books = localStorage.getItem(this.BOOKS_KEY);
      const parsedBooks = books ? JSON.parse(books) : this.getDefaultBooks();
      console.log('📚 ReadingService: Books loaded:', parsedBooks.length, 'items');
      return parsedBooks;
    } catch (error) {
      console.error('❌ ReadingService: Error getting books:', error);
      return this.getDefaultBooks();
    }
  }

  // Get reading progress (alias for getBooks for compatibility with analytics service)
  getReadingProgress() {
    console.log('📚 ReadingService: getReadingProgress() called');
    const books = this.getBooks();
    
    // Ensure each book has progress calculated
