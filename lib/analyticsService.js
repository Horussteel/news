class AnalyticsService {
  ...
  // Get dashboard data (for Dashboard component)
  getDashboardData() {
    try {
      const habitMetrics = this.getHabitPerformanceMetrics();
      const moodAnalytics = this.getMoodAnalytics();
      ...
      return {
        overview: {
          totalHabits: habitMetrics.activeHabits,
          consistencyScore: habitMetrics.consistencyScore,
          currentStreak: wellnessReport.mood.currentStreak || 0,
          wellnessScore: wellnessReport.overallScore.score
        },
        recentActivity: this.getRecentActivity(),
        topMetrics: {
          bestHabit: habitMetrics.bestPerformingHabit,
          moodTrend: moodAnalytics.recentTrend,
          weeklyProgress: habitMetrics.weeklyProgress.slice(-1)[0],
          monthlyTrend: habitMetrics.monthlyTrend
        },
        quickStats: {
          habitsCompleted: this.calculateHabitsCompleted(),
          moodEntries: wellnessReport.mood.totalEntries || 0,
          currentWeekProgress: this.getCurrentWeekProgress(),
          improvementAreas: wellnessReport.recommendations.filter(r => r.priority === 'high').length
        }
      };
    } catch (error) {
      console.error('❌ AnalyticsService: Error getting dashboard data:', error);
      return {
        overview: { totalHabits: 0, consistencyScore: 0, currentStreak: 0, wellnessScore: 0 },
        recentActivity: [],
        topMetrics: { bestHabit: null, moodTrend: 'stable', weeklyProgress: null, monthlyTrend: 'stable' },
        quickStats: { habitsCompleted: 0, moodEntries: 0, currentWeekProgress: 0, improvementAreas: 0 }
      };
    }
  }
  ...
  getRecentActivity() {
    try {
      ...
      habits.forEach(habit => {
        const habitCompletions = completions[habit.id] || [];
        if (Array.isArray(habitCompletions) && habitCompletions.includes(today)) {
          activities.push({
            type: 'habit',
            title: `Completat: ${habit.name}`,
      ...
      // Recent mood entry
      const moodEntries = moodService.getMoodEntries(1);
      if (moodEntries.length > 0) {
        const latestMood = moodEntries[0];
        activities.push({
          type: 'mood',
          title: `Stare emoțională: ${latestMood.mood}`,
          time: latestMood.date,
          icon: latestMood.emoji || '😊'
        });
      }
  ...
  getCurrentWeekProgress() {
    try {
      const habitMetrics = this.getHabitPerformanceMetrics();
      const currentWeek = habitMetrics.weeklyProgress.slice(-1)[0];
      return currentWeek ? currentWeek.completionRate : 0;
    } catch (error) {
      return 0;
    }
  }
